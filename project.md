...
1. **채우기와 획순 경로**: 각 `*Strokes.ts`의 동일한 정규화 좌표를 사용해 모양 불일치를 방지
2. **글자 채우기**: 과목별 연한 색상의 굵은 선으로 따라 쓸 영역 표시
3. **획순 경로**: 핑크 점선 (`#ec4899`)으로 진행 방향 표시
4. **시작 번호**: 경로 시작점 가까이에 작은 핑크 숫자로 표시
5. **방향 화살표**: 각 획의 진행 방향을 핑크 삼각형으로 표시
6. **교차점 분리**: 서로 다른 획이 붙어 보이지 않도록 끝점을 자동으로 안쪽 조정

큰 번호 배지와 번호 연결선은 사용하지 않으며, 한글/영어/숫자 모두 같은 시각 규칙을 사용한다.
...
- **자음/모음 탭**: 큰 캔버스 + 데이터 기반 채우기/획순 가이드 + 따라쓰기/자유쓰기 모드 + 자동 채점
- **단어 탭**: 이모지 + 단어 표시 → 음절별 큰 독립 캔버스로 따라쓰기 (1~2글자)
- 현재 글자를 처음 표시할 때 읽어주고, 버튼으로 다시 들을 수 있음
- 쓰기 완료를 감지하면 한 항목씩 자동 진행하며, 지연 콜백의 중복 진행을 방지

### 영어 글씨 쓰기 (`/english/writing`)
- **대/소문자 탭**: A→a→B→b 순서, 큰 캔버스 + 데이터 기반 채우기/획순 가이드
- **첫 글자 탭**: 이모지와 단어를 보고 첫 글자를 큰 캔버스에 쓰기
- 현재 글자/단어를 처음 표시할 때 읽어주고, 버튼으로 다시 들을 수 있음
- 쓰기 완료를 감지하면 한 항목씩 자동 진행하며, 지연 콜백의 중복 진행을 방지
- 알파벳 퀴즈는 새 문제가 표시될 때 해당 글자를 자동으로 읽어줌
- 파닉스 페이지 코드는 유지하지만 영어 홈에서는 메뉴 카드를 숨김

### 숫자 쓰기 (`/math/number-writing`)
- **0~9 탭**: 큰 캔버스 + 데이터 기반 채우기/획순 가이드
- **1~100 탭**: 숫자 표시 → 자릿수별 큰 독립 캔버스 (1자리=1칸, 2자리=2칸, 100=3칸)
- 현재 숫자를 처음 표시할 때 읽어주고, 버튼으로 다시 들을 수 있음
- 모든 필요한 캔버스의 쓰기 완료를 감지하면 한 항목씩 자동 진행
...
- 채우기와 획순 경로는 반드시 동일한 `*Strokes.ts` 좌표를 사용
- 획 번호는 작고 선명한 핑크 텍스트로 표시하며 번호원/연결선은 사용하지 않음
